...
public interface SidukMapper
{
	...
	@Select("select kel.nama_kelurahan, kel.id, kel.kode_kelurahan, kec.nama_kecamatan, kec.id, kota.id, kec.kode_kecamatan, kota.kode_kota, kota.nama_kota "
	...
	@Update("update penduduk set nik = #{nik}, nama = #{nama}, tempat_lahir = #{tempat_lahir}, tanggal_lahir = #{tanggal_lahir}, "
			+ "id_keluarga=#{id_keluarga}, jenis_kelamin = #{jenis_kelamin}, is_wni = #{is_wni}, "
...
//	 List<PendudukModel> selectPendudukByIdKelurahan(int id_kelurahan);
	 ...
	 @Select ("select p.nik, p.nama, p.tanggal_lahir from penduduk p, keluarga klg, "
	 		+ "kelurahan kl, kecamatan kc"
	 		+ " where p.id_keluarga=klg.id AND klg.id_kelurahan=kl.id AND kl.id_kecamatan=kc.id and kc.id=#{id_kecamatan} "
	 ...
	 @Select ("select p.nik, p.nama, p.tanggal_lahir from penduduk p, keluarga klg, "
		 ...
		 PendudukModel selectPendudukMudaaByIdKecamatan(int id_kecamatan);
	 ...
	 //bonus cari tertua di kelurahan
	 
	 @Select ("SELECT p.nama as nama, p.nik as nik, p.tanggal_lahir from penduduk p, keluarga klg, kelurahan kl "
	 		+ "where p.id_keluarga=klg.id AND klg.id_kelurahan=kl.id and kl.id=#{id_kelurahan} "
	 		+ "ORDER BY tanggal_lahir ASC LIMIT 1")
	PendudukModel selectTua(@Param("id_kelurahan")int id_kelurahan);
	 
	 @Select ("SELECT p.nama as nama, p.nik as nik, p.tanggal_lahir  "
	 		+ "from penduduk p, keluarga klg, kelurahan kl, kecamatan kc"
